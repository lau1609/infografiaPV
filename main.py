...
def generar_pie_chart_b64(labels: list, valores: list) -> str:
    """Genera un gráfico de pastel y lo convierte a String Base64."""
...
async def generar_infografia(data: PayloadInfografia):
    try:
        ...
        # 1. Filtrar preguntas que NO tienen respuestas y reacomodar
        preguntas_validas = [p for p in data.preguntas if len(p.respuestas) > 0]
        
        # 2. Control de desborde (Ajustar font-size dinámicamente)
        ...
        for preg in preguntas_validas:
            # Evaluar presencia de nacionalidad en los datos
        ...
        if es_especial:
            # Datos para el pie chart (ejemplo estructurado)
        ...
        # 6. Capturar Screenshot con Playwright (Sin scrollbar)
